...
// IMPORTANT: Apply rate limiting to ALL API routes EXCEPT health
app.use('/api', rateLimiter);
...
const connectWithRetry = async () => {
  try {
    ...
    console.log('✅ RabbitMQ connected successfully');
    console.log(`📦 Queue: ${RABBITMQ_QUEUE}`);
    console.log(`🚦 Rate limit: 50 requests per minute per IP`);
...
const server = app.listen(PORT, '0.0.0.0', () => {
  console.log(`✅ API server running on port ${PORT}`);
  console.log(`🔍 Health check: http://localhost:${PORT}/health`);
  console.log(`🚦 Rate limiting: 50 requests/minute/IP enabled`);
